...
Career-OS is designed to be customized by Claude itself. Just ask Claude to change anything and it will edit the right files.
...
**Quick way:** Just tell Claude "change the archetypes to [your roles]" and it will update everything.
...
The original modes are bilingual (Spanish/English). English aliases are provided:
- `evaluate` = `oferta`
- `compare` = `ofertas`
- `outreach` = `contacto`

To translate all modes to English, tell Claude: "Translate the modes to English"
...
Career-OS can integrate with external systems via Claude Code hooks:
...
```json
{
  "hooks": {
    "SessionStart": [{
      "hooks": [{
        "type": "command",
        "command": "echo 'Career-OS session started'"
      }]
    }]
  }
}
